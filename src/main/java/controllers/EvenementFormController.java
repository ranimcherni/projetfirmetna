...
public class EvenementFormController {
    ...
    public void initialize() {
        ...
        if (generationStatusLabel != null) {
            generationStatusLabel.setVisible(false);
            generationStatusLabel.setManaged(false);
        }
    }

    private void generateDescriptionAsync(String eventName) {
        setGeneratingState(true);
        CompletableFuture.supplyAsync(() -> {
            try {
                return huggingFaceService.generateEventDescription(eventName);
            } catch (Exception e) {
    ...
    private void setGeneratingState(boolean isGenerating) {
        if (generateDescriptionBtn != null) {
            generateDescriptionBtn.setDisable(isGenerating);
        }
        if (generationStatusLabel != null) {
            generationStatusLabel.setText("Generation en cours...");
            generationStatusLabel.setVisible(isGenerating);
            generationStatusLabel.setManaged(isGenerating);
        }
    }
    ...
    private void handleGenerateDescription() {
        hide(descError);
        String eventName = nomField.getText() == null ? "" : nomField.getText().trim();
        if (eventName.isEmpty()) {
            show(descError, "Saisissez d'abord le nom de l'evenement.");
            return;
        }
        if (descriptionArea.getText() != null && !descriptionArea.getText().trim().isEmpty()) {
            show(descError, "La description existe deja. Videz le champ pour regenerer.");
            return;
        }
        generateDescriptionAsync(eventName);
    }
    ...
    private void handleAiError(Throwable ex) {
        ...
        String msg = cause.getMessage() != null ? cause.getMessage() : "Erreur lors de la generation automatique.";
        show(descError, "IA: " + msg);
    }
