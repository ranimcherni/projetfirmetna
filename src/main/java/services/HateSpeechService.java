...
public class HateSpeechService {
    ...
    public Result analyze(String text) {
        // Fallback: Local Keyword Check if API key is missing
        if (apiKey == null || apiKey.isBlank()) {
            System.out.println("HateSpeechService: API key missing, using local keyword filter.");
            String lower = text.toLowerCase();
            String[] forbidden = {"hate", "bad person", "stupid", "idiot", "haine", "méchant", "naze", "imbécile"};
            for (String word : forbidden) {
                if (lower.contains(word)) {
                    return new Result(true, 0.99, "LOCAL_FILTER_" + word.toUpperCase());
                }
            }
        ...
        try {
            ...
            if (status != 200) {
                System.err.println("HateSpeechService: HTTP " + status + " from Perspective API. Falling back to local filter.");
                // Also apply local filter on API error
    ...
    private Result analyzeLocal(String text) {
        String lower = text.toLowerCase();
        String[] forbidden = {"hate", "bad person", "stupid", "idiot", "haine", "méchant", "naze", "imbécile"};
        for (String word : forbidden) {
            if (lower.contains(word)) {
